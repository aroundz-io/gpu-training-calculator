...
function orderDataBytes() {
  // 업로드 전체(혼합 포함)를 보관 대상으로 계산, 파일이 없으면 직접 입력 모달리티 기준
  const total = order.textBytes + order.imgBytes + (order.otherBytes || 0);
  return total > 0 ? total : (order.dataType === "text" ? order.textBytes : order.imgBytes);
}
...
dz.addEventListener("drop", async e => {
  ...
  // 폴더 드롭 지원: webkitGetAsEntry로 하위 디렉터리까지 재귀 수집
...
$("folderInput").addEventListener("change", e =>
  addFiles([...e.target.files].map(f => ({ raw: f, path: f.webkitRelativePath || f.name }))));
...
async function walkEntry(entry, path, out) {
  if (entry.isFile) {
    const f = await new Promise((r, j) => entry.file(r, j)).catch(() => null);
    if (f) out.push({ raw: f, path: path + f.name });
  } else if (entry.isDirectory) {
    const reader = entry.createReader();
    let batch;
    do {
      batch = await new Promise((r, j) => reader.readEntries(r, j)).catch(() => []);
      for (const en of batch) await walkEntry(en, path + entry.name + "/", out);
    } while (batch.length);
  }
}

// ---------- ZIP 자동 해제 (외부 라이브러리 없이 중앙 디렉터리 파싱 + DecompressionStream) ----------
async function listZip(file) {
  try {
    ...
    if (cdOfs === 0xFFFFFFFF || count === 0xFFFF) return null;   // ZIP64 미지원
    ...
    for (let i = 0; i < count && p + 46 <= cd.length; i++) {
      ...
      if (!name.endsWith("/") && usize > 0 && !name.startsWith("__MACOSX")) {
        entries.push({ name, size: usize, csize, method, lho });
      }
      p += 46 + nlen + elen + clen;
    }
...
async function extractZipEntry(zipFile, en, maxBytes = 8e6) {
  try {
    if (en.size > maxBytes) return null;
    const lh = new DataView(await zipFile.slice(en.lho, en.lho + 30).arrayBuffer());
    if (lh.getUint32(0, true) !== 0x04034b50) return null;
    const nlen = lh.getUint16(26, true), elen = lh.getUint16(28, true);
    const start = en.lho + 30 + nlen + elen;
    const comp = zipFile.slice(start, start + en.csize);
    if (en.method === 0) return comp;                       // 무압축(stored)
...
async function addFiles(list) {
  for (const it of list) {
    const f = it.raw || it;
    const name = it.path || f.webkitRelativePath || f.name;
    const isFile = (typeof File !== "undefined" && f instanceof File);
    if (fileExt(name) === "zip" && isFile) {
      $("fileList").innerHTML = `<div class="note">${icon("box","sm")} ${name} — 압축 해제·분류 중…</div>`;
      const entries = await listZip(f);
      if (entries) {
        entries.forEach(en => order.files.push({
          name: name + "/" + en.name, size: en.size, file: null, zsrc: { zip: f, entry: en },
        }));
        continue;
      }
...
// 콘텐츠 접근 통합: 실제 File 또는 ZIP 내부 항목
async function readBlob(f) {
  if (f.file) return f.file;
  if (f.zsrc) return await extractZipEntry(f.zsrc.zip, f.zsrc.entry);
  return null;
}
...
async function deepAnalyze() {
  ...
  const texts = order.files.filter(f => (f.file || f.zsrc) && TEXT_EXT.includes(fileExt(f.name))).slice(0, 6);
  const imgs  = order.files.filter(f => (f.file || f.zsrc) && IMG_EXT.includes(fileExt(f.name))).slice(0, 8);
  ...
  try {
    if (texts.length) {
      ...
      for (const f of texts) {
        const blob = await readBlob(f);
        if (!blob) continue;
        const txt = await blob.slice(0, 2e6).text();   // 파일당 앞 2MB 샘플
...
function renderAnalysis() {
  ...
  const zipN = order.files.filter(f => f.zsrc).length;
  if (zipN) rows.push(["압축 자동 해제", "ZIP 내부 " + fmtNum(zipN, 0) + "개 파일 분류 완료"]);
  if (order.textBytes > 0 && order.imgCount > 0) {
    rows.push(["혼합 구성", fmtBytes(order.textBytes) + " 텍스트 · " + fmtNum(order.imgCount, 0) + "장 이미지 (" + fmtBytes(order.imgBytes) + ")" +
      (order.otherBytes > 0 ? " · 기타 " + fmtBytes(order.otherBytes) : "")]);
...
function syncStep1() {
  ...
  if (has) {
    ...
    $("dsType").innerHTML = mixed
      ? icon("folder","sm") + " 혼합 (" + (isText ? "텍스트 학습" : "이미지 학습") + ")"
      : isText ? icon("file","sm") + " 텍스트" : icon("image","sm") + " 이미지";
    $("dsTypeSub").innerHTML = mixed
      ? `학습 대상 선택: <span class="seg" style="vertical-align:middle;margin-left:2px">
          <button class="${isText ? "on" : ""}" onclick="setModality('text')">텍스트</button>
          <button class="${!isText ? "on" : ""}" onclick="setModality('image')">이미지</button></span>`
      : (manual ? "직접 입력 기준" : "업로드 파일 자동 판별");
